...
const SimilarTvShows = (props) => {
    ...
    const getSimilarShows = async () => {
        ...
        const response = await fetch (`https://api.themoviedb.org/3/tv/${movieId}/similar?api_key=fae62ec587f8bc50f34e46b6d33b41e2&language=en-US&page=1`)
        if (response.ok===true){
    ...
    const renderSimilarMovies = () => (
            <>
                <h1 className='similar-movies-heading'>Similar TV Shows</h1>
                <ul className='similar-movies-list'>
                {similarMoviesList.map(movie => (
                    <Link to={`/tv/${movie.id}`} key={movie.id} className="link-style">
